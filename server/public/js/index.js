...
function _classCallCheck(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("Cannot call a class as a function"); } } // 由lottery继承所有的模块，进行实例化
// 需要服务端给时间戳，联调完成
    ...
    try {
        for (var _iterator = Reflect.ownKeys(source)[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
            ...
            if (key !== 'constructor' && key !== 'prototype' && key !== 'name') {
                var desc = Object.getOwnPropertyDescriptor(source, key); // 获取属性
...
var Lottery = function (_mix) {
    ...
    function Lottery() {
        ...
        _this.issue_el = '#curr_issue'; // 当前期号选择器
        _this.countdown_el = '#countdown'; // 倒计时选择器
        _this.state_el = '.state-el'; // 状态选择器
        _this.cart_el = '.codelist'; // 购物车选择器
        _this.omit_el = ''; // 遗漏选择器
        _this.cur_play = 'r5'; //默认玩法
        _this.initPlayList(); // 初始化玩法列表
        _this.initNumber(); // 初始化？？
    ...
    _createClass(Lottery, [{
        ...
        value: function initEvent() {
            ...
            (0, _jquery2.default)('.dxjo').on('click', 'li', self.assignHandle.bind(self));
...
var Base = function () {
    ...
    _createClass(Base, [{
        ...
        value: function changePlayNav(e) {
            ...
            (0, _jquery2.default)('.boll-list .btn-boll').removeClass('btn-boll-active'); // 移除上次选中的号码
            self.getCount();
        }
        ...
        value: function assignHandle(e) {
            ...
            var self = this;
            var $cur = (0, _jquery2.default)(e.currentTarget);
            var index = $cur.index(); // 返回当前选中集合的索引，表示全、大、小等
            (0, _jquery2.default)('.boll-list .btn-boll').removeClass('btn-boll-active'); // 清除之前的选项
            if (index === 0) {
                // 全选
        ...
        value: function addCode() {
            var self = this;
            var $active = (0, _jquery2.default)('.boll-list .btn-boll-active').text().match(/\d{2}/g); // 当前选中号码的值，数组形式
        ...
        value: function getCount() {
            var self = this;
            var active = (0, _jquery2.default)('.boll-list .btn-boll-active').length; // 选中号码的个数
            var count = self.computeCount(active, self.cur_play);
        ...
        value: function getRandom(num) {
            var arr = [],
                index = void 0;
            var number = Array.from(this.number); //Set转Array，集合转数组l
            while (num--) {
        ...
        value: function getRandomCode(e) {
            ...
            var play = this.cur_play.match(/\d+/g)[0]; // 获取当前玩法
            var self = this;
            if (num === '0') {
...
var Timer = function () {
    ...
    _createClass(Timer, [{
        ...
        value: function countdown(end, update, handle) {
            ...
            var self = this; //当前对象指针
            if (now - end > 0) {
                handle.call(self); // 倒计时结束的回调
            } else {
                var last_time = end - now; // 剩余时间
                var px_d = 1000 * 60 * 60 * 24; //一天的ms数
                var px_h = 1000 * 60 * 60; // 一小时的ms数
                var px_m = 1000 * 60; // 一分钟的ms数
                var px_s = 1000;
...
var Calculate = function () {
    ...
    _createClass(Calculate, [{
        ...
        value: function computeCount(active, play_name) {
            var count = 0; // 当前注数
            var exist = this.play_list.has(play_name); // play_list是Map类型  判断是否有对应玩法
            // arr是用来保存所选号码的数组
            var arr = new Array(active).fill('0'); // active是数组长度，fill()填充数组
        ...
        value: function computeBonus(active, play_name) {
            ...
            var play = play_name.split(''); // 当前玩法的基数 ['r','3']
            ...
            (function f(arr, size, result) {
                ...
                if (size === arrLen) {
                    ...
                    for (var i = 0; i < arrLen; i++) {
                        var newResult = [].concat(result); // 生成新的数组
...
var _createClass = function () { function defineProperties(target, props) { for (var i = 0; i < props.length; i++) { var descriptor = props[i]; descriptor.enumerable = descriptor.enumerable || false; descriptor.configurable = true; if ("value" in descriptor) descriptor.writable = true; Object.defineProperty(target, descriptor.key, descriptor); } } return function (Constructor, protoProps, staticProps) { if (protoProps) defineProperties(Constructor.prototype, protoProps); if (staticProps) defineProperties(Constructor, staticProps); return Constructor; }; }(); // 接口功能
...
var Interface = function () {
    ...
    _createClass(Interface, [{
        ...
        value: function getOmit(issue) {
            var self = this; // 当前引用，箭头函数的this指向是在定义而非运行时的，内部引用self
            return new Promise(function (resolve, reject) {
